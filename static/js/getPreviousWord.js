function getPreviousWord()
{
    ...
    st.value = "Getting previous word.  Please wait."; 
    ...
    var url = "http://" + hostAndPort + "/previous_word/" + ws.value;
    ...
    if (!request)
    {
        st.textContent = "new XMLHttpRequest() returned null.  Call Patrick.";
        return;
    }
        ...
        if (request.readyState == XMLHttpRequest.DONE)
        {
            if (request.status == 200)
            {
                ...
                try 
                {
                    ...
                    ms.value = ''
                    md.value = ''
                    st.value = "Previous word was found."
                }
                catch(err) 
                {
            ...
            {
                st.value = "Previous word was not found." 
            }
            else
            {
